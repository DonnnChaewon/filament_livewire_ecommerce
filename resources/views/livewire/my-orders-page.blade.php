                        ...
                        <tbody>
                            @forelse ($orders as $order)
                                @php
                                    ...
                                    if($order->status == 'processing') $status = '<span class="bg-yellow-500 py-1 px-3 rounded text-white shadow">Processing</span>';
                                    if($order->status == 'shipped') $status = '<span class="bg-green-500 py-1 px-3 rounded text-white shadow">Shipped</span>';
                                    if($order->status == 'delivered') $status = '<span class="bg-green-700 py-1 px-3 rounded text-white shadow">Delivered</span>';
                                    if($order->status == 'cancelled') $status = '<span class="bg-red-500 py-1 px-3 rounded text-white shadow">Cancelled</span>';

                                    if($order->payment_status == 'pending') $payment_status = '<span class="bg-blue-500 py-1 px-3 rounded text-white shadow">Pending</span>';
                                    if($order->payment_status == 'paid') $payment_status = '<span class="bg-green-600 py-1 px-3 rounded text-white shadow">Paid</span>';
                                    if($order->payment_status == 'failed') $payment_status = '<span class="bg-red-600 py-1 px-3 rounded text-white shadow">Failed</span>';
                                @endphp
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4 text-4xl font-semibold text-slate-500">You have no orders.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
